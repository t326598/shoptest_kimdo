...
public class UserRepository extends JDBConnection {
	...
	public User login(String id, String pw) {
	    ...
	    try {
	        ...
	        if (rs.next()) {
	            ...
	            user.setMail(rs.getString("mail")); // 'maul'이 아니라 'mail'
	 ...
	 * @param userId
	 */
	public PersistentLogin refreshToken(String userId) {
	    ...
	    if (persistentLogin == null) {
	        ...
	        persistentLogin.setUserId(userId);
	        persistentLogin.setToken(token);
	        persistentLogin.setDate(new Date()); // 현재 시간 설정
	    } else {
	        // 기존 토큰이 있으면 갱신
	        token = updateToken(userId);
	        persistentLogin.setToken(token);
	        persistentLogin.setDate(new Date()); // 갱신 시간 설정
	    }
	    return persistentLogin; // PersistentLogin 객체 반환
	}
